...
export const init = async (workspace, apiKey, experienceShortCode = null) => {
    global.workspaceUrl = validWorkspaceUrl(workspace);
    global.apiKey = apiKey;
    global.experienceShortCode = experienceShortCode;
...
export const isTablePushMessageJPush = (pushNotification) => {
    if (pushNotification && pushNotification.extras) {
        return pushNotification.extras.hasOwnProperty("table_id")
    } else {
        return false
    }
}
...
export const updateJPushRegistrationID = async (registrationID, androidNotificationChannel) => {
    const response = await AsyncStorage.getItem('data');
    const asyncData = JSON.parse(response);
    if (asyncData.user.token) {
        return await API.updateJPushRegistrationID(global.workspaceUrl, registrationID, asyncData.user.token, androidNotificationChannel)
    } else {
        console.log("Couldn't get auth token");
    }
}
