...
export const init = async (workspace, apiKey, experienceShortCode = null) => {
  ...
  global.backgroundColor = await API.getInstallationProperties(
    global.workspaceUrl,
  );
  console.log('New background col', global.backgroundColor);
  console.log(global.workspaceUrl);
};
...
export const updateFCMToken = async (fcmToken, androidNotificationChannel) => {
  ...
  if (asyncData.user.token) {
    return await API.updateFcmToken(
      global.workspaceUrl,
      fcmToken,
...
export const updateAPNSToken = async (apnsToken) => {
  ...
  const asyncData = JSON.parse(response);
  if (asyncData.user.token) {
    return await API.updateAPNSToken(
      global.workspaceUrl,
      apnsToken,
      asyncData.user.token,
    );
...
export const updateJPushRegistrationID = async (
  ...
  const asyncData = JSON.parse(response);
  if (asyncData.user.token) {
    return await API.updateJPushRegistrationID(
      global.workspaceUrl,
      registrationID,
      asyncData.user.token,
      androidNotificationChannel,
