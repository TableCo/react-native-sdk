...
export const updateJPushRegistrationID = async (registrationID) => {
    const response = await AsyncStorage.getItem('data');
    const asyncData = JSON.parse(response);
    if (asyncData.user.token) {
        return await API.updateJPushRegistrationID(global.workspaceUrl, registrationID, asyncData.user.token)
    } else {
        console.log("Couldn't get auth token");
    }
}
