...
export class Dashboard extends Component {
    ...
    constructor(props) {
        ...
        console.log(`TableSDK - Dashboard constructed, navigator in props ${(this.props.navigator)}`)
    }
    ...
    componentDidMount = async () => {
        ...
        console.log(`TableSDK - Dashboard didMount`)

        AsyncStorage.getItem('data').then(res => {
            let asyncData = JSON.parse(res);
            if (asyncData) {
                console.log(`TableSDK - Dashboard got data from storage`)
    ...
    onHeaderLeftIconOrBackNav = (navigation) => {
        const {backButtonNavigatesBrowser} = this.state

        if (backButtonNavigatesBrowser) {
            this.goBack()
        } else {
            this.props.navigation.navigate('Welcome') //TODO: Needs to be a pop() or 'isFinished' callback on the SDK
        }
    };
    ...
    onNavigationStateChange = async (navState) => {
        try {
            ...
            console.log("TableSDK - Error getting conversation title", err)
        }
    ...
    onNewConversation = async (navigation) => {
        try {
            ...
            console.log("TableSDK - Created new conversation ", newConversationId)
            ...
            console.log("TableSDK - Error creating conversation", err)
        ...
        return (
            <View style={{flex: 1}}>
                <Header
                    ...
                    onLeftIconPress={() => this.onHeaderLeftIconOrBackNav(this.props.navigation)}
