...
export class Dashboard extends Component {
    ...
    onHeaderLeftIconPress = (navigation) => {
        const {urlpage, backButtonNavigatesBrowser} = this.state

        // console.log(`Header left icon pressed ${urlpage} enabled ${backButtonNavigatesBrowser}`)
    ...
    onNavigationStateChange = async (navState) => {
        ...
        const newMessageEnabled = (navState.url.endsWith('/conversation'))
        // console.log("Navigation state", navState)
        // console.log("new message enabled", newMessageEnabled)
    ...
    onLocationChange = async (url) => {
        ...
        const newMessageEnabled = (url.endsWith('/conversation'))
    ...
    checkUrlForHeaderTitle = async (url) => {
        ...
        if (url && url.includes('/conversation/')
            ...
            const tableId = url.substring(url.lastIndexOf('/') + 1)
            if (tableId && tableId !== '') {
                console.log("Getting conversation title ", tableId)
    ...
    render() {
        const {asyncData, headerTitle, initialUrl, blockUi, token, newMessageButtonEnabled} = this.state;
        return (
            <View style={{flex: 1}}>
                <Header
                    isLeftIcon={true}
